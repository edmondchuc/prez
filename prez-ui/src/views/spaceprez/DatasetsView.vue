...
const { data: datasetData, loading: datasetLoading, error: datasetError, doRequest: doDatasetRequest } = useGetRequest();

onMounted(() => {
    doDatasetRequest(`${apiBaseUrl}/s/datasets`, () => {
        parseIntoStore(datasetData.value);
        ...
        store.value.forObjects(member => {
            ...
            store.value.forEach(q => { // get preds & objs for each subj
                if (q.predicate.value === qname("rdfs:label")) {
                    ...
                    c.id = q.object.value;
                    c.link = `/spaceprez/dataset/${q.object.value}`;
                }
            }, member, null, null);
            datasets.value.push(c);
...
<template>
    ...
    <div>
        <ItemList v-if="datasetData" :items="datasets" />
        <template v-else-if="datasetLoading">loading...</template>
        <template v-else-if="datasetError">Network error: {{ datasetError }}</template>
    </div>
</template>

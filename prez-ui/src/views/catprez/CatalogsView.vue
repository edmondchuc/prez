...
const { data: catalogData, loading: catalogLoading, error: catalogError, doRequest: doCatalogRequest } = useGetRequest();

onMounted(() => {
    doCatalogRequest(`${apiBaseUrl}/c/catalogs`, () => {
        parseIntoStore(catalogData.value);
        ...
        store.value.forObjects(member => {
            ...
            store.value.forEach(q => { // get preds & objs for each subj
                if (q.predicate.value === qname("rdfs:label")) {
                    ...
                    c.id = q.object.value;
                    c.link = `/catprez/catalog/${q.object.value}`;
                }
            }, member, null, null);
...
<template>
    ...
    <div>
        <ItemList v-if="catalogData" :items="catalogs" />
        <template v-else-if="catalogLoading">loading...</template>
        <template v-else-if="catalogError">Network error: {{ catalogError }}</template>
    </div>
</template>
